...
apiRouter.post('/cookie/:name/:value', (req, res, next) => {
  res.cookie(req.params.name, req.params.value);
  res.send({cookie: `${req.params.name}:${req.params.value}`});
});

apiRouter.post('/user/create/:username/:password', async (req, res) => {
...
apiRouter.post('/user/recipe', async (req, res) => {
  const body = req.body;
...
apiRouter.get('/user/recipe/:username', async (req, res) => {
  const userRecipes = await DB.getRecipesByUser(req.params.username);
  res.send({ recipes: userRecipes});
});
apiRouter.get('/user/recipe/', async (req, res) => {
  const recipes = await DB.getRecipes();
  res.send({ recipes: recipes});
});
apiRouter.get('/user/group/:groups/:username/:auth', (req, res) => {
  res.send({ groups: ''});
});

apiRouter.get('/user/chats/:chats/:username/:auth', (req, res) => {
  res.send({ chats: ''});
});

apiRouter.get('/user/notifications/:notifications/:username/:auth', (req, res) => {
